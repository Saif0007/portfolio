...
export const projects: Project[] = [
  ...
  {
    title: "Pioneer License Verification Agent",
    slug: "pioneer-license-verification-agent",
    description:
    ...
    pipeline: [
      "Salesforce Apex trigger fires on Job Applicant stage change to 'Submitted to Account Manager' when License_Verified__c = FALSE",
      "LicenseVerificationCallout Apex class POSTs job_applicant_id + contact_id to backend /webhook/verify-license with HMAC secret",
      "Backend returns 200 immediately; verification runs as a detached asyncio background task — no Salesforce timeout risk",
      "Backend fetches JA + Contact from Salesforce concurrently (discipline, setting, license state(s), license number)",
      "3-key site_key is built: <discipline>_<setting>_<state>. Multi-state candidates get one verification job per state. Clinical disciplines auto-merge national registry (ANY) entries on top of state results",
      "Backend POSTs to scraper /verify; scraper orchestrator resolves the site_key to 1–3 target URLs from url_map.json",
      "Playwright launches: CAPTCHA-protected sites (Cloudflare, Turnstile, reCAPTCHA, hCaptcha) get a fresh Bright Data residential session; plain boards share a local Chromium",
      "Each scraper navigates the board, searches by name + license number, extracts status and expiration date, and captures a full-page PDF screenshot as compliance evidence",
      "PDF is uploaded to S3 (pioneer-license-pdfs bucket) and a 7-day presigned URL is returned",
      "Results are saved to Supabase, License__c records are created in Salesforce with status + expiration, PDF is attached as a Salesforce file",
      "License_Verified__c on the Job Applicant is set TRUE if any result is active + not expired. Dashboard notifications fire for success, CAPTCHA block, or no-record outcomes",
    ],
    roles: [
      {
        name: "Admin",
        description: "Pioneer compliance team leads — full platform access including user management.",
        permissions: [
          "View all verification jobs with status, candidate name, discipline, state, and site key",
          "Drill into job detail pages with per-site results and PDF evidence links",
          "View full verifications list with filter/search",
          "Access analytics dashboard — job volume, pass rate, CAPTCHA rate, site breakdown",
          "Manage users (create Viewer accounts, reset credentials)",
          "View and clear notifications",
        ],
      },
      {
        name: "Viewer",
        description: "Read-only compliance team members — can monitor activity without modifying anything.",
        permissions: [
          "View verification jobs and results",
          "View PDF evidence and verification outcomes",
          "View analytics and notification feed",
